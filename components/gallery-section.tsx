...
function VideoItem({ item, isVisible, index }: { item: typeof galleryItems[0]; isVisible: boolean; index: number }) {
  ...
  return (
    <div 
      className={`relative group overflow-hidden rounded-lg cursor-pointer ${item.span} transition-all duration-700 ${
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className={`w-16 h-16 rounded-full bg-primary/90 flex items-center justify-center transition-all duration-300 ${isPlaying ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}>
          {isPlaying ? (
      ...
      <div className="absolute inset-0 flex items-end p-6">
        <div className="transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
          <span className="text-xs font-mono text-primary uppercase tracking-wider">Video</span>
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
        <div className="absolute inset-0 rounded-lg shadow-[0_0_30px_rgba(0,255,100,0.3)]" />
      </div>
    </div>
  )
}
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <p className="text-muted-foreground mt-4 max-w-2xl mx-auto">
            Registros das performances mais marcantes e da energia que so o DJ KPO consegue criar
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px] md:auto-rows-[250px]">
          {galleryItems.map((item, index) => (
            item.type === "video" ? (
              <VideoItem key={index} item={item} isVisible={isVisible} index={index} />
            ) : (
              <div 
                key={index}
                className={`relative group overflow-hidden rounded-lg cursor-pointer ${item.span} transition-all duration-700 ${
